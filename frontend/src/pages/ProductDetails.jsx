...
const ProductDetails = () => {
  ...
  // ✅ Whether this product has a minimum order quantity
  const requiresMinQty = product && MIN_QTY_PRODUCTS.includes(product.name);
  const minQty = requiresMinQty ? 2 : 1;
  ...
  const hasMultipleImages = () => getProductImages().length > 1;

  const getPrice = () => {
    if (!product || !selectedWeight) return 0;
    const priceMap = product.pricePerWeight || {};
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col">
            ...
            {requiresMinQty && (
              <div className="mb-6 p-3 bg-amber-50 border-2 border-amber-400 rounded-lg">
                <p className="text-sm text-amber-800 font-semibold">
                  ℹ️ Minimum order quantity: {minQty}
                </p>
                <p className="text-xs text-amber-700 mt-1">
                  You need to buy at least {minQty} of this item to place an order.
                </p>
              </div>
            )}

            {/* Weight / Length Selection */}
